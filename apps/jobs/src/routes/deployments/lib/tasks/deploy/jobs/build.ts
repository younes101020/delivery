...
export async function build(job: QueueDeploymentJob<"build">) {
  ...
  const deployCmd = `nixpacks build ./ --name ${repoName} --env CI=false ${!cache ? "--no-cache" : ""} -l "resource=application" -l "traefik.enable=true" -l "traefik.http.routers.${repoName}.rule=Host(\\\`${fqdn}\\\`)" -l "traefik.http.services.${repoName}.loadbalancer.server.port=${port}" && docker run ${env ?? ""} --network host_network -d ${repoName}`;
