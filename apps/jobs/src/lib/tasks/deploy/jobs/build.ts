...
export const build: JobFn<"build"> = async (job) => {
  ...
  await job.updateProgress({ logs: "Image will be built..." });

  const systemconfig = await db.query.systemConfig.findFirst();
  const hostName = systemconfig?.domainName;
  if (!hostName) {
    await job.updateProgress({ logs: "No domain name configured in system settings" });
    await job.remove();
  }
  ...
  await ssh.execCommand(
    `nixpacks build ./ \
      --name ${repoName} \
      --label "traefik.http.routers.${repoName}.rule=Host(\\\`${fqdn}\\\`)" \
      --label "traefik.http.routers.${repoName}.entrypoints=web" \
      && docker run ${port} ${env ?? ""} --network host_network -d ${repoName}`,
    {
